...
采用技巧是，第一个子进程fork另一个子进程来执行程序并先于第二个子进程终止。由于第一个子进程的父进程调用了waitpid，他不
会遗留在系统的僵死进程。而第二个子进程尽管其父进程没有调用wait等待它，但由于父进程先于它而终止使得它被init所继承。这使得
它是活跃的进程，当它终止时，init将调用wait释放其proc结构，因而不会成为僵死进程。
