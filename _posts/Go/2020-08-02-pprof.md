...
kubectl exec -it -n rec-prod filter-canary-767b48795f-s4472 -- /bin/sh

进入指定pod 的shell命令行，执行```wget 127.0.0.1:16060/management/debug/pprof/profile?seconds=30```

可以用cat命令下载
```
kubectl exec -n rec-prod recall-canary-7f8f7698ff-zrm95 -- cat /app/profile\?seconds=30 >  profile.pprof 
```
